...
// goalieMode: "pairs" | "halves"
// shiftsPerGame: number (applies to all games) OR array of 4 numbers (per-game)
// killedShifts: { "g-s": true } — shifts to skip entirely; algorithm leaves the slot null
export function generateRotation(seed, players, shiftExclusions = {}, gameExclusions = {}, shiftForceIns = {}, lockedShifts = {}, shiftForcePositions = {}, goalieMode = "pairs", shiftsPerGame = 8, killedShifts = {}) {
  const rand = mulberry32(seed);
  const sgArr = Array.isArray(shiftsPerGame) ? shiftsPerGame : [shiftsPerGame, shiftsPerGame, shiftsPerGame, shiftsPerGame];
  ...
  players.forEach(p => {
    ...
    for (let g = 0; g < 4; g++) {
      ...
      const allShiftsExcluded = Array.from({ length: sg }, (_, i) => i).every(s => {
        const key = `${g}-${s}`;
  ...
  function buildGoalieSchedule(gameIndex, availPlayers, sg) {
    const sh = sg / 2;
    const ag = availPlayers.filter(p => p.canGoalie).map(p => p.name);
    if (ag.length === 0) return Array(sg).fill(null);
    if (ag.length === 1) return Array(sg).fill(ag[0]);
    ...
    if (goalieMode === "halves") {
      ...
      return [...Array(sh).fill(gk1), ...Array(sh).fill(gk2)];
    }

    // Pairs: rotate every 2 shifts SEQUENTIALLY (no half-boundary alignment).
    // Fixed offset based on max possible pairs (8 shifts -> 4 pairs) so the
    // first N shifts of a game are identical regardless of total length:
    //   4-shift: [gk[0], gk[0], gk[1], gk[1]]
    //   6-shift: [gk[0], gk[0], gk[1], gk[1], gk[2], gk[2]]
    //   8-shift: [gk[0], gk[0], gk[1], gk[1], gk[2], gk[2], gk[3], gk[3]]
    const MAX_PAIRS = 4;
    const offset = gameIndex * MAX_PAIRS;
    const schedule = [];
    for (let s = 0; s < sg; s++) {
      const pairIdx = Math.floor(s / 2);
      schedule.push(sorted[(offset + pairIdx) % sorted.length]);
    }
    return schedule;
  }
  ...
  for (let g = 0; g < 4; g++) {
    ...
    const gs = buildGoalieSchedule(g, avail, sgArr[g] || 8);
    allGameGK.push(gs);
  ...
  for (let g = 0; g < 4; g++) {
    ...
    for (let s = 0; s < sg; s++) {
      const shiftKey = `${g}-${s}`;
      const isKilled = !!killedShifts[shiftKey];
      const locked = lockedShifts[shiftKey];
      if (locked) {
        const shift = { ...locked };
        if (!isKilled) updateState(shift, shift.G, gamePlayers, pShiftsGame, pHistory, totalShifts, totalPositions);
        game.push(isKilled ? { ...shift, _killed: true } : shift);
        continue;
      }
      ...
      // If the next shift's goalie is a different player, keep them off the field
      // this shift so they have time to put their gear on.
      const nextShiftGoalie = (s + 1 < sg) ? goalieShifts[s + 1] : null;
      const available = gamePlayers.filter(p =>
        p.name !== goalie &&
        !shiftEx.includes(p.name) &&
        p.name !== nextShiftGoalie
      );
      ...
      if (available.length <= fieldSize || slotsToFill <= 0) {
        ...
        if (!isKilled) updateState(shift, goalie, gamePlayers, pShiftsGame, pHistory, totalShifts, totalPositions);
        game.push(isKilled ? { ...shift, _killed: true } : shift); continue;
      }
      ...
      for (const combo of combos) {
        ...
        for (const p of combo) {
          ...
          // Soft penalty for back-to-back field shifts
          const hist = pHistory[p.name] || [];
          if (hist.length >= 1 && hist[hist.length - 1] === 'play') score -= 120;
          score -= shifts * 20;
        ...
        for (const p of notSel) {
          ...
          const sLeft = sg - (s + 1); const needed = 3 - totalExp; const fieldOpp = sLeft - futGK;
